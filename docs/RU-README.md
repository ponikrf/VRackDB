...
Последнее обновление 2.3.0
--------------------------
 * Добавлен метод `Collector.start(metricId)` который возвращает предпологаемое начало графика метрики
 * Добавлен метод `Collector.end(metricId)` который возвращает предпологаемый конец графика метрики
 * Добавлен метод `Collector.writeCount(metricId)` который возвращает количество записываний в метрику
 * Теперь размер для каждый метрик вычисляется заранее, а `Collector` возвращает уже готовое значение. Это сильно ускоряет подсчет занимаемого места метрик в схеме
 * Добавлен метод `Database.readAll(metricId, precision, func)` Который вовзращает данные с начала записи метрики и до последней записи
