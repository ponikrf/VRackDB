...
ErrorManager.register('WLzlG5xV8FxE', 'VDB_BASIC_CONDITION_TYPE', 'Invalid condition type')
ErrorManager.register('bLbncRI79a9g', 'VDB_BASIC_CONDITION_PARAMS', 'Invalid parameter type or number')
...
export default class BasicCondition extends AlertCondition {
    ...
    */
    constructor(level: string, type: keyof typeof EConditionType, params: Array<number>) {
        ...
        if (type === "inRange" || type === "outRange") if (params[0] > params[1]) params.reverse()

        const id = Utility.uid()
    ...
    */
    check(value: number | null, id: string) {
        if (value === null) {
            if (this.condition.type === EConditionType.noValue) return true
            return false
        }
        switch (this.condition.type) {
            case EConditionType.isAbove:
