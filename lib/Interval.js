...
class Interval {
    ...
    */
    static toTime(jsTime) {
        if (!Number.isInteger(jsTime))
            throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_TIME', { time: jsTime });
    ...
    */
    static roundTime(time, precision) {
        if (!Number.isInteger(time))
            throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_TIME', { time });
        if (!Typing_1.default.isUInt(precision))
            throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_PRECISION', { precision });
    ...
    */
    static getIntervals(start, end, precision) {
        if (start > end)
            throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_SECTION', { start, end });
    ...
    */
    static period(period, pieces = {}) {
        period = period.replace(/ /g, '');
        if (period.indexOf(':') === -1)
            throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_PERIOD', { period });
    ...
    */
    static partOfPeriod(str, pieces = {}) {
        str = str.replace(/ /g, '');
    ...
    static retentions(str) {
        ...
        if (str.indexOf(',') !== -1) {
            ...
            for (const sret of retons)
                result.push(this.retention(sret));
        }
        else {
    ...
    */
    static parseInterval(ival) {
        ...
        if (!match || !this.map[match[2]]) {
            ...
            if (isNaN(r)) {
                throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_PARSE', { interval: ival });
            }
            return r;
        }
    ...
    */
    static prepareInterval(str, pieces = {}) {
        if (str === 'now')
            pieces.now = this.now();
    ...
    */
    static retention(ret) {
        const acts = ret.split(':');
        if (acts.length !== 2)
            throw ErrorManager_1.default.make(new Error, 'VDB_INTREVAL_RETENTION', { retention: ret });
        const newRet = {
