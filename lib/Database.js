...
class Database {
    ...
    */
    readAll(name, precision, func = 'last') {
        if (!this.Collector.has(name))
            return __classPrivateFieldGet(this, _Database_instances, "m", _Database_readFake).call(this, Interval_1.default.now(), Interval_1.default.now(), 1);
        const start = this.Collector.start(name);
        const end = this.Collector.end(name);
        if (start === 0 || start > end)
            return __classPrivateFieldGet(this, _Database_instances, "m", _Database_readFake).call(this, start, start, 1);
