    ...
    */
    constructor(level, type, params) {
        ...
        if (EConditionType[type] === undefined)
            throw ErrorManager_1.default.make('VDB_BASIC_CONDITION_TYPE', { type });
        if (params.length !== CTypeParamsCount[type])
            throw ErrorManager_1.default.make('VDB_BASIC_CONDITION_PARAMS', { count: params.length });
        for (let i = 0; i < params.length; i++) {
            if (typeof params[i] !== "number") {
                throw ErrorManager_1.default.make('VDB_BASIC_CONDITION_PARAMS', { count: params.length, index: i });
            }
        }
